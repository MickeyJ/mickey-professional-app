...
const FeatureCard: React.FC<FeatureCardProps> = ({ icon, title, description, href, linkText, colorScheme }) => {
  const colorClasses = {
    blue: {
  ...
  return (
    <div className="feature-card flex flex-col h-full">
      <div
        className={cn("w-12 h-12 rounded-lg flex items-center justify-center mb-6", colorClasses[colorScheme].iconBg)}
      >
        {icon}
      </div>
      ...
      <div className="mt-auto">
        <Link href={href} className={colorClasses[colorScheme].link}>
          {linkText} →
        </Link>
      </div>
    </div>
  ...
  const features = [
    {
      icon: <SlidersIcon />,
      title: "Theme",
      description: "Modify this site's appearance in real-time. Change colors, fonts, and layout with live previews.",
      href: "/demo/theme",
      linkText: "Try it",
    ...
    },
    {
      icon: <ChartIcon />,
      title: "Data",
      description: "Explore interactive charts and graphs powered by real APIs with customizable parameters.",
      href: "/demo/data",
      linkText: "Visualize",
